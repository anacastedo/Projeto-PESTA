 ...
 * @param {any[]} orderedEquations equations from supernodes
 * @param equationsV1
 * @returns {string} HTML string
 */
function outSupernodesTex(supernodes, orderedEquations, equationsV1){
 ...
 * @returns {string} HTML string
 */
function outCurrentsInfoTex(currents, branches){
    // TEX Data
    let TeXData = '';
...
const printDoc = (targetLnk, targetBtn, TeX, canvasObjects) => {
    //Get User info
    ...
    //Print TeX (Temporary - Index 1686 - texfile cannot be changed before it)
    if(studNumber.length>1 && studLastname.length > 1 && studNumber.length>1){
        ...
        if(language === "english"){
            TeX = TeX.slice(0, 1686) + string + TeX.slice(1687);}
        else{
            TeX = TeX.slice(0, 1707) + string + TeX.slice(1708);
        }
    }
    // Instanciate printer object
    docToPrint = new latexprinter(null, targetLnk, targetBtn);
    // Add the desired Latex Source Code
    docToPrint.setTexFile(TeX);
    // Add Logo Image
    let sampleimg = base64imgselect("logo");
    docToPrint.addImgFile('logo.png', sampleimg);
    // Add Circuit Image
    if(fileContents[0]){
        let imageObj = new Image();
        imageObj.src = fileContents[0];
        sampleimg = resizeandgray(imageObj);
        docToPrint.addImgFile('circuit.png', sampleimg);
    }
    // Add Canvas Images
    for(let i = 0; i< canvasObjects.length; i++){
        docToPrint.addImgFile(canvasObjects[i].id+'.png',canvasObjects[i].dataURL)
    }

    console.log('TEXFile', TeX);

    return {TeX, docToPrint}
}
 ...
 * @returns {string} HTML string
 */
function outEqImpedancesTex(Zequiv, Vequiv, Nodes){
    ...
    // Create panel
    if(TeXData !== "\\subsection{Equivalent Impedances and Voltages}\r\n"){
        return TeXData;
    }
    else
        TeXData = '';
 ...
 * @param {Array} simpEquations simplified equation system
 ...
 */
function outEquationSystemTex(simpEquations,strStep1, strStep2, strStep3, strStep4, strStep5, strStep6){
    let TeXData = "";
 ...
 */
function outCurrentsKNLTex(knlCurrEquations,supernodes) {
    let TeXData = "";
    ...
    for (let i = 0; i < knlCurrEquations.length; i++) {
        ...
        if (obj.currents.filter(function (x) {
            return x.flow === "in";
        }).length === 0)
            strEq += '0='
        else
 ...
 * @returns {string} HTML string
 */
function outResultsTex(results, currents, lang){
    let TeXData = "";
    ...
    str += '\\end{cases}';
    // Parse system to TeX
    TeXData += "\\subsection{"+ lang._nodeVoltages +"}\r\n\\begin{gather*}\r\n" + str + "\r\n\\end{gather*}\r\n\r\n";
    ...
    TeXData += "\\subsection{"+ lang._branchCurrents +"}\r\n";
...
function getTexFileHeader(lang, pdf = true){
    let texHeader;
...
function output( props ) {
    console.log(props.canvasObjects)
    // Tex Variable
    let TeX = ''

    if (fileContents[0]) {
        TeX += "\\section{" + props.lang._circuitImage + "}\r\n\r\n\\begin{figure}[hbt]\r\n\\centering{\\resizebox{12cm}{!}{";
        TeX += "\\inlineimages{circuit.png}{\\circuit}}}\r\n\\caption{";
        TeX += "" + props.lang._circuitImage + "}\r\n\\label{circuitimage}\r\n\\end{figure}\r\n\r\n";
    }

    // TeX Fundamental Vars
    let N = props.nodeCnt - 1 - props.isolatedPsReg.length;
    let I = props.acAmpsPs.length + props.dcAmpsPs.length;
    TeX += "\\section{" + props.lang._fundamentalsTitle + "}\r\n\r\n\\begin{table}[hbt!]\r\n\\centering\r\n\\begin{tabular}{clclclc}\r\n";
    TeX += "\\textbf{" + props.lang._fundamentals_R + " {[}R{]}}&&\\textbf{" + props.lang._fundamentals_N + " {[}N{]}}&&\\textbf{" + props.lang._fundamentals_T + " {[}T{]}}&&\\textbf{" + props.lang._fundamentals_E + " {[}E{]}} \\\\\r\n";
    TeX += "R=" + props.branches.length + "&&N=" + props.nodeCnt + "&&T=" + props.isolatedPsReg.length + "&&E=N-T-1=" + N + "\r\n\\end{tabular}\r\n\\end{table}\r\n\r\n";

    // TeX Circuit Information
    TeX += "\\section{" + props.lang._infoTitle + "}\r\n\r\n\\begin{table}[h!]\r\n\\centering\r\n\\begin{tabular}{clclclc}\r\n";
    TeX += "\\textbf{" + props.lang._info_F + " {[}F{]}} &  & \\textbf{" + props.lang._fundamentals_C + " {[}I{]}} &  & \\textbf{" + props.lang._info_A + " {[}A{]}} &  & \\textbf{" + props.lang._info_T + " {[}AC\/DC{]}} \\\\\r\n";
    TeX += "F=" + props.circuitAnalData.frequency.value + "\\;" + props.circuitAnalData.frequency.mult + " & & I=" + I;
    TeX += " & & " + props.ampsMeters.length + "\/" + props.currents.length + " & &";
    if (props.circuitAnalData.frequency.value === 0)
        TeX += "DC\r\n\\end{tabular}\r\n\\end{table}\r\n\r\n\\pagebreak";
    else
        TeX += "AC\r\n\\end{tabular}\r\n\\end{table}\r\n\r\n\\pagebreak";

    // TeX Data
    if (props.supernodes.length > 0)
        TeX += "\\section{" + props.lang._snTitle + "}\r\n\r\n" + props.supernodesOutputTex + "\\pagebreak";
    TeX += "\\section{" + props.lang._circuitCurr + "}\r\n\r\n\\subsection{" + props.lang._genInfo + "}\r\n\r\n";
    TeX += "\\begin{table}[ht]\r\n\\caption{" + props.lang._currentsTableCap + "}\r\n\\centering\r\n\\begin{tabular}{cccc}\r\n";
    TeX += "\\textbf{" + props.lang._reference + "} & \\textbf{" + props.lang._startNode + "} & \\textbf{" + props.lang._endNode + "} & \\textbf{" + props.lang._components + "} \\\\ \\hline\r\n";
    TeX += props.currentsInfoOutputTex + "\\end{tabular}\r\n\\end{table}\r\n\r\n";
    TeX += props.equivImpOutputTex + "\\pagebreak";
    TeX += "\\subsection{" + props.lang._barsEquations + "}\r\n" + props.lang._KNLtext + "\r\n\r\n" + props.knlCurrDataTex;
    TeX += "\\pagebreak\\section{" + props.lang._eqSystemTitle + "}\r\n\r\n\\paragraph{} " + props.equationSystemOutputTex;
    TeX += "Steps:\r\n\r\n" + props.step1.second + props.step2.second + props.step3.second + props.step4.second + props.step5.second + props.step6.second;
    TeX += "\\par\r\n\r\n\\pagebreak\r\n\r\n\\section{" + props.lang._resultsMCR + "}\r\n\r\n" + props.resultsOutputTex;
    TeX += "\\end{document}\r\n";


    // Export JSON File (Working)
    $("#json").off().on('click', function() {
        const filename = 'urisolve_results.json';
        let element = document.createElement('a');
        element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(JSON.stringify(props.jsonFile)));
    ...
    // Open in overleaf(Working
    $("#overleaf").off().on('click', function() {
        let TeX_output = printDoc(null, null, getTexFileHeader(props.lang, false) + TeX, props.canvasObjects).TeX
        let ImagesTeX = buildImTeX(props.canvasObjects);

        TeX_output = TeX_output.replaceAll("[latin1]", "");
        document.getElementById('main').value = encodeURIComponent(TeX_output);
        document.getElementById('images').value = encodeURIComponent(ImagesTeX);
        document.getElementById('overleaf').submit();
    });

    // Export PDF file (not working)
    // $("#print").off().on('click', function() {
    //     printDoc('null', 'print', getTexFileHeader(props.lang, true,) + TeX, props.canvasObjects).docToPrint.downloadPdf()
    // });
    $("#print").off().on('click', function() {
        buildPrintPDFMTN(props.jsonFile, props.canvasObjects);
    });
...
function outCircuitFundamentals(R, N, T){
    ...
    let btnStyle = ["btn-results", "btn-results", "btn-results", "btn-results"];
    ...
    for(let i = 0; i<4; i++){
        ...
        htmlstr += '<div class="accordion" id="'+accIDs[i]+'"><div class="card-results">';
...
function outCircuitInfo(F, Asources, Amps, totalCurrents){
    ...
    if(Amps < totalCurrents){
        ...
        htmlstr += '<div class="alert alert-dismissible fade show" style="background-color: white; border-left: 6px solid #8CE85A;">';
...
function outSupernodes(supernodes, orderedEquations, equationsV1){
    ...
    if(supernodes.length > 0){
        htmlstr += '<div class="container mt-3"><div class="row bg-light rounded text-dark p-2">';
...
function outCurrentsInfo(currents, branches){
    ...
    for( let i = 0; i < currents.length; i++){
        ...
        htmlstr += '<div class="card-results text-dark bg-light mb-3">';
...
function outEqImpedances(Zequiv, Vequiv, Nodes){
    ...
    // Create panel
    if(htmlstr.length > 1){
        let row = '<div class="container mt-3"><div class="row bg-light rounded text-dark p-2">';
...
function outCurrentsKNL(knlCurrEquations,supernodes){
    ...
    if(knlCurrEquations.length > 0){
        htmlstr += '<div class="container mt-3"><div class="row bg-light rounded text-dark p-2"><h5 class="ml-3" data-translate="_knlTitle"></h5></div></div>';
 ...
 * @param {Array} equations KNL Equations (without substitutions)
 ...
 * @param {Array} equations KNL Equations (with substitutions)
 * @param {object} realNodes circuit nodes
 * @returns {string} HTML string
 */
...
function outStep6(supernodes, unknowns){
    ...
    htmlstr += '<div class="p-1" style="background-color: white; border-left: 6px solid #ffeb3b;">';
...
function outEquationSystem(simpEquations,strStep1, strStep2, strStep3, strStep4, strStep5, strStep6){
    ...
    if(simpEquations.length > 0){
        ...
        htmlstr += '<div class="container mt-3">';
        htmlstr += '<div class="row bg-light rounded text-dark p-2"><h5 class="ml-3" data-translate="_eqSystemTitle"></h5></div></div>';
        ...
        // STEP #1
        btnstr  = '<button class="btn collapsed border text-dark bg-light btn-info btn-sm float-right mt-1 mb-1 mr-1" ';
        ...
        htmlstr += '<div class="row bg-primary rounded">';
        // Add step text
 ...
 * @returns {string} HTML string
 */
function outResults(results, currents, lang){
    ...
    htmlstr += '<div class="card-header rounded text-light bg-info d-flex align-items-center justify-content-center" style="opacity:0.9">';
    ...
    htmlstr += '<div class="card-header rounded text-light bg-info d-flex align-items-center justify-content-center no-page-break" style="opacity:0.9">';
    ...
    if(ACCurrents.length > 0){
        ...
        str = katex.renderToString(str, {throwOnError: false});
        // Add Notes
        htmlstr += '<div class="card p-1 mb-0 no-page-break" style="background-color: white; border-left: 6px solid #ffeb3b;">';
    ...
    if(directCurrents.length > 0){
        ...
        str = katex.renderToString(str, {throwOnError: false});
        // Add Notes
        htmlstr += '<div class="card p-1" style="background-color: white; border-left: 6px solid #ffeb3b;">';
        htmlstr += '<div class="container-fluid"><div class="d-flex flex-row">';
        htmlstr += '<div class="ml-1 mt-1"><i class="fas fa-sticky-note"></i></div>';
 ...
 * @param {Array} equations equation strings
 * @returns {Array} new equations
 */
 ...
 * @param {Array} realNodes circuit nodes
...
function outHTMLSections(){
    ...
    htmlstr += '<div class="container mt-3">';
    htmlstr += '<div class="row bg-light rounded text-dark p-2"><h5 class="ml-3" data-translate="_fundamentalsTitle"></h5></div></div>';
    htmlstr += '<div class="container mt-3" id="fundamentalVars"></div><div class="container mt-3">';

    // Circuit information
    htmlstr += '<div class="row bg-light rounded text-dark p-2"><h5 class="ml-3" data-translate="_infoTitle"></h5></div></div>';
    ...
    // Currents data
    htmlstr += '<div class="container mt-3">';
    htmlstr += '<div class="row bg-light rounded text-dark p-2"><h5 class="ml-3" data-translate="_currents"></h5></div></div>';
    ...
    // Results
    htmlstr += '<div class="container mt-3">';
    htmlstr += '<div class="row bg-light rounded text-dark p-2"><h5 class="ml-3" data-translate="_res"></h5></div></div>';
...
//TODO: not implemented 22/05/2023
/**
 * Main function to build the pdf for printing
 * @param {object} file output jsonFile
 * @param {array} nodeImages array with the images information
 */
function buildPrintPDFMTN(file, nodeImages){
    ...
    //print head
    //doc = printBuildHeadMTN(doc, lang);

    doc.addPage();
    //doc = printBuildFootMTN(doc, marginSides, marginBottom, marginTop, lang);
    ...
    // line = printEqSystemMTN(doc, file, line, marginSides, marginTop, marginBottom, lang);
