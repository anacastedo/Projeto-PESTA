 ...
 *
 * Wrapper based on the great work of Manuels:
 ...
 * @param {number} printerId    Id of the printer (optional)
...
class latexprinter{
    constructor(printerId, targetLnk, targetBtn) {
        this.id = printerId || null,
            ...
            /**
             * Set Latex Source Code
             * @param {string} texSourceFile  File containing an unminified latex source code
             */
            this.setTexFile = function(texSourceFile) {
                this.latexCode = texSourceFile;
            },
            /**
             * Add Image files
             * @param {string} filename  Name of the file (must be the same used in Latex Source Code)
             * @param {string} base64img Image in base64 format (accepts jpg, png, pdf...)
             */
             ...
             * Clean Image files
             */
            this.clearImgRepo = function() {
                this.imgArr = [];
            },
            /**
             * Create pdf file and prompt it to be downloaded (using the targetlink dom object) when ready
             * @returns {number} errorCode 0 - Success | -1 Latex Source Code not found
             */
            this.print = function() {
                let errorCode = 0;
...
var texwork = function(opt_workerPath) {
    if (!opt_workerPath) {
        opt_workerPath = 'vendor/latexprinter/texworker.js';
    }
